...
class Simulator:
    ...
    @accepts(str, int)
    def __init__(self, address=env.str("LGSVL__SIMULATOR_HOST", "localhost"),
                 port=env.int("LGSVL__SIMULATOR_PORT", 8181)):
                    ...
                    if event_type in callbacks:
                        for fn in callbacks[event_type]:
                            if event_type == "collision":
                                fn(agent, self.agents.get(ev["other"]),
                                   Vector.from_json(ev["contact"]) if ev["contact"] is not None else None)
    ...
    def weather(self):
        ...
        return WeatherState(j.get("rain", 0), j.get("fog", 0), j.get("wetness", 0), j.get("cloudiness", 0),
                            j.get("damage", 0))
    ...
    def weather(self, state):
        self.remote.command("environment/weather/set", {"rain": state.rain, "fog": state.fog, "wetness": state.wetness,
                                                        "cloudiness": state.cloudiness, "damage": state.damage})
    ...
    def map_from_gps_batch(self, coords):
        ...
        for c in coords:
            ...
            if ("latitude" in c and c["latitude"] is not None) and ("longitude" in c and c["longitude"] is not None):
                if not isinstance(c["latitude"], numtype): raise TypeError(
                    "Argument 'latitude' should have '{}' type".format(numtype))
                if not isinstance(c["longitude"], numtype): raise TypeError(
                    "Argument 'longitude' should have '{}' type".format(numtype))
                ...
                if not isinstance(c["northing"], numtype): raise TypeError(
                    "Argument 'northing' should have '{}' type".format(numtype))
                if not isinstance(c["easting"], numtype): raise TypeError(
                    "Argument 'easting' should have '{}' type".format(numtype))
            ...
            if "altitude" in c and c["altitude"] is not None:
                if not isinstance(c["altitude"], numtype): raise TypeError(
                    "Argument 'altitude' should have '{}' type".format(numtype))
                j["altitude"] = c["altitude"]
            if "orientation" in c and c["orientation"] is not None:
                if not isinstance(c["orientation"], numtype): raise TypeError(
                    "Argument 'orientation' should have '{}' type".format(numtype))
    ...
    def raycast_batch(self, args):
        ...
        for hit in hits:
            if hit is None:
                ...
                results.append(
                    RaycastHit(hit["distance"], Vector.from_json(hit["point"]), Vector.from_json(hit["normal"])))
